...
param(
    ...
    [Parameter(Mandatory=$false)][string]$ArtifactsRoot = "<set me to the gallerly link after publishing 1.5.0 to the marketplace>",
    [Parameter(Mandatory=$false)][string]$DockerRepository = $null,
...
$azWebApp = Get-Command "Get-AzWebApp" -ErrorAction SilentlyContinue
if ($azWebApp -eq $null -and ($rmWebApp.Source -ne "AzureRM.Websites.Netcore" -and $rmWebApp.Version.Major -lt 5))
{
    throw "The required version of the Azure Powershell cmdlets was not detected. We recommend that you follow the
    instructions on https://docs.microsoft.com/en-us/powershell/azure/overview?view=azurermps-6.8.1 to update to a compatible version. Or,
...
function UpgradeInitBlob( $orig)
{
    ...
    foreach ($entry in $json)
    {
        ...
        if($entry.environment.DOCKER_REPOSITORY -and $DockerRepository -ne $null)
        {
            $entry.environment.DOCKER_REPOSITORY = $DockerRepository
        }

        if($entry.environment.DOCKER_PASSWORD -and $DockerPw -ne $null)
        {
            $entry.environment.DOCKER_PASSWORD = $DockerPw
        }

        if($entry.environment.DOCKER_LOGIN -and $DockerLogin -ne $null)
        {
...
ForEach($setting in $apiWebsite.SiteConfig.AppSettings)
{
    ...
    if($setting.Name -eq "DOCKER_REGISTRY_SERVER_URL" -and $DockerRepository -ne $null)
    {
        $setting.Value = "$DockerRepository"
    }

    if($setting.Name -eq "DOCKER_REGISTRY_SERVER_PASSWORD" -and $DockerPw -ne $null)
    {
        $setting.Value = "$DockerPw"
    }

    if($setting.Name -eq "DOCKER_REGISTRY_SERVER_USERNAME" -and $DockerLogin -ne $null)
    {
